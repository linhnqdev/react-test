...
export interface IMealCategoryButtonsProps {
  selected?: MealType | null;
  onSelect?: (type: MealType | null) => void;
}

// Custom SVG icon for utensils (fork and knife crossed)
const UtensilsIcon: React.FC = () => (
  <svg width="56" height="56" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg">
    <rect opacity="0.01" width="56" height="56" fill="white"/>
      <g clip-path="url(#clip0_77128_68)">
      <g clip-path="url(#clip1_77128_68)">
...
export function MealCategoryButtons({ selected: selectedProp, onSelect }: IMealCategoryButtonsProps) {
  const [internalSelected, setInternalSelected] = useState<MealType | null>(null);
  const selected = selectedProp !== undefined ? selectedProp : internalSelected;

  const categories: Array<{ type: MealType; label: string; icon: React.ReactNode }> = [
    { type: "morning", label: "Morning", icon: <UtensilsIcon /> },
    { type: "lunch", label: "Lunch", icon: <UtensilsIcon /> },
    { type: "dinner", label: "Dinner", icon: <UtensilsIcon /> },
    { type: "snack", label: "Snack", icon: <SnackIcon /> },
  ];

  const handleClick = (type: MealType) => {
  ...
  return (
    <div className={styles.container}>
      {categories.map((category) => (
        <button
          key={category.type}
          className={`${styles.hexButton} ${selected === category.type ? styles.active : ""}`}
